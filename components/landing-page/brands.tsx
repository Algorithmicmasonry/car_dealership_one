...
interface BrandsProps {
  brands: ContentDataResponse["brands"]
}
...
export default function Brands({ brands }: BrandsProps) {
  ...
  // If no brands data, don't render the component
  if (!brands || brands.length === 0) {
    return null
  }

  // Split brands into two rows for mobile carousel
  ...
  const [showFirstRowLeftArrow, setShowFirstRowLeftArrow] = useState<boolean>(false)
  const [showFirstRowRightArrow, setShowFirstRowRightArrow] = useState<boolean>(true)

  // State for second row arrows
  const [showSecondRowLeftArrow, setShowSecondRowLeftArrow] = useState<boolean>(false)
  const [showSecondRowRightArrow, setShowSecondRowRightArrow] = useState<boolean>(true)
  ...
  return (
    <section
      id="brands"
      className="w-screen h-full rounded-xl md:rounded-2xl max-w-[1440px] mx-auto bg-primary-blue py-6 px-8 mt-[80px]"
    >
      <div className="px-[20px]">
        <h1 className="pt-[70px] font-bold text-white text-3xl">Cars Available For Sale</h1>
        <p className="text-white pt-[25px] text-lg">
          Looking to buy a car? Search through our dealership of premium cars at your fingertips, certified by experts
          and friendly to your pocket
        </p>
        <p className="text-white text-md pt-[35px] text-xl md:text-3xl">Explore Our Premium Brands</p>

        {/* Mobile carousel view - First Row */}
        {firstRowBrands.length > 0 && (
            ...
            >
              {firstRowBrands.map((brand, index) => (
                ...
                >
                  <div>
                    <Image
                      src={brand.image || "/placeholder.svg?height=50&width=50"}
                      alt={`${brand.name} logo`}
                      width={50}
                      height={50}
                      priority
                  ...
                  <div className="text-xl hover:text-primary-blue">{brand.name}</div>
                </div>
              ))}
            </div>
